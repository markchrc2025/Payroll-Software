 ...
 *
 * Mail clients DROP relative paths, so every <img> must use an absolute URL on
 * a CDN. Upload the files in docs/design_handoff_emails/assets/ to your bucket
 * and point EMAIL_ASSET_BASE_URL at the folder that holds them.
 *
 * Resolution order:
 *   1. EMAIL_ASSET_BASE_URL                (explicit, preferred)
 *   2. R2_PUBLIC_URL + "/email-assets"     (reuse the app's R2 public domain)
 *   3. https://assets.sentire.solutions/email-assets   (last-resort default)
 *
 * Expected files at the base (exact names — see README "Assets"):
