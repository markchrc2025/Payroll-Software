 ...
 * DELETE /api/departments/[id] — Soft-delete (409 if employees assigned)
 */
...
export async function PATCH(
  ...
  const result = await withTenant(auth.tenantId, async (tx) => {
    ...
    return {
      ...
      row: await tx.department.update({ where: { id }, data: parsed.data }),
    };
  });

  if (result.notFound) return notFound("Department not found");
  if (result.conflict) return err("Department name already taken", 409);
...
export async function DELETE(
  ...
  const result = await withTenant(auth.tenantId, async (tx) => {
    ...
    return {
      ...
      row: await tx.department.update({
        where: { id },
  ...
  if (result.notFound) return notFound("Department not found");
  if (result.inUse)
    return err("Cannot delete: employees are assigned to this department", 409);
  void writeAuditLog({
    tenantId: auth.tenantId,
  ...
  return ok({ id }, "Department deleted");
}
