/**
 * GET /api/payroll/reports/sss/r3?year=YYYY&month=M
 *
 * Returns SSS R-3 (Monthly Collection List) data for the given calendar year
 * and month.  Only FINALIZED PayrollBooks whose `periodEnd` falls within the
 * requested month are included.
 *
 * Response: { data: SssR3Report }  — includes `ecsText` for ECS file upload.
 */
...
} from "@/lib/payroll/reports/sss-r3";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const report = await withTenant(auth.tenantId, async (tx) => {
      ...
      if (sheets.length === 0) {
        return buildSssR3Report({
          year, month, tenantId: auth.tenantId, tenantName: tenant.name,
          employees: [],
        });
      }

      const byEmployee = new Map<string, SssR3SheetInput[]>();
      ...
      const employeeInputs: SssR3EmployeeInput[] = employees.map((e) => ({
        employeeId: e.id,
