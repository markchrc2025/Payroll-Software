/**
 * GET /api/payroll/reports/philhealth/rf1?year=YYYY&month=M
 *
 * Returns PhilHealth RF1 (Employer's Remittance Report) data for the given
 * calendar year and month.  Only FINALIZED PayrollBooks whose `periodEnd`
 * falls within the requested month are included.
 *
 * Response: { data: PhilhealthRf1Report }
 */
...
} from "@/lib/payroll/reports/philhealth-rf1";
...
/** Extract philHealthMscCents from the statutoryBreakdown JSON field. */
...
export async function GET(req: NextRequest) {
  ...
  try {
    const report = await withTenant(auth.tenantId, async (tx) => {
      ...
      if (sheets.length === 0) {
        return buildPhilhealthRf1Report({
          year, month, tenantId: auth.tenantId, tenantName: tenant.name,
          employees: [],
        });
      }

      const byEmployee = new Map<string, PhilhealthRf1SheetInput[]>();
      ...
      const employeeInputs: PhilhealthRf1EmployeeInput[] = employees.map((e) => ({
        employeeId: e.id,
    ...
    console.error("[philhealth/rf1]", e);
