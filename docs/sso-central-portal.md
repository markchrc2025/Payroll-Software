...
Single sign-on for the **Central Portal** (`/centralportal/login`) using
**Authenticize** (our own identity platform), Google Workspace, or Microsoft
Entra ID.
...
### Authenticize (first-party)
| Variable | Value |
|----------|-------|
| `AUTH_AUTHENTICIZE_ISSUER` | `https://authenticize.sliplane.app` (or its custom domain later) |
| `AUTH_AUTHENTICIZE_ID` | Client ID generated by the Authenticize dashboard |
| `AUTH_AUTHENTICIZE_SECRET` | Client secret (shown once at creation; rotatable) |
...
- Authenticize: `https://<app-domain>/api/auth/callback/authenticize`
...
4. Copy the generated client ID/secret into `AUTH_AUTHENTICIZE_ID` /
   `AUTH_AUTHENTICIZE_SECRET`, and set
   `AUTH_AUTHENTICIZE_ISSUER=https://authenticize.sliplane.app`.
5. Add `authenticize` to `NEXT_PUBLIC_CENTRAL_SSO` and redeploy.
6. The signing-in person must already exist as a **Central admin** here *and*
   as a user on Authenticize (it's invite-only) with the **same email**.
